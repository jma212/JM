...
CSprojectHat::FirstStage(){
	...
	data=data[1:][]~150;                         // here I create data that generate empirical moment to match	
	objsim= new SMM("SMM", data, hat1, hat2);	 // the object compute the distance
...
//	hat1=new StDeviations("logw",DGP1);
	...
	AuxiWage= new AuxiliaryValues("Wage");
...
CSprojectHat::Run(){
	...
	decl stages=10~max(Ttotal-10-3)~3;
    SubSampleStates(ones(1,stages[0])~constant(0.15,1,stages[1])~constant(0.1,1,stages[2]));
	Method=new KeaneWolpin();
	Method->Solve();
	ps=new Panel(0, Method, TRUE);
	ps->Simulate(Nsim, Ttotal);
//	decl newd=ps->Print(1);
	ps->Flat();
	simdata=ps.flat;
...
//	decl newd=ps->Print(1);
	...
	R=(laborsupply-labordemand)|vecr(momentscs)|vecr(momentsncs);
//	println("*****************",R);
	return R; 
	delete Method, ps;
    }
